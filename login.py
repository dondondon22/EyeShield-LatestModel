...
import os
import json
from PySide6.QtWidgets import QWidget, QLabel, QPushButton, QLineEdit, QVBoxLayout, QHBoxLayout, QCheckBox, QMessageBox, QDialog, QFrame, QTextEdit, QScrollArea, QTableWidget, QTableWidgetItem, QStyle
...
class ContactAdminDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        fields = [('NAME', contact.get('name', '')), ('EMAIL', contact.get('email', '')), ('PHONE', contact.get('phone', '')), ('LOCATION', contact.get('location', ''))]
    ...
    def _open_email(self):
        ...
        if self._email:
            QDesktopServices.openUrl(QUrl(f'mailto:{self._email}?subject=EyeShield%20Account%20Request'))
...
class LoginWindow(QWidget):
    ...
    def handle_login(self):
        ...
        if self.lockout_remaining_seconds > 0:
            QMessageBox.warning(self, 'Login Locked', f'Too many failed attempts. Please wait {self.lockout_remaining_seconds} seconds.')
            return
        username = self.username_input.text().strip()
        role = verify_user(username, self.password_input.text())
        if role:
            self.failed_attempts = 0
            ...
            try:
                ...
                pass
            self.main = EyeShieldApp(username, role, display_name=display_name, full_name=full_name, specialization=specialization, contact=contact)
            self.main.show()
            ...
            QMessageBox.warning(self, 'Login Failed', f'Invalid credentials. You have {remaining_attempts} attempt(s) remaining.')
    ...
    def _start_lockout(self):
        ...
        QMessageBox.warning(self, 'Too Many Attempts', f'Too many failed login attempts. Login is locked for {self.LOCKOUT_SECONDS} seconds.')
    ...
    def closeEvent(self, event):
        if self._allow_close_without_prompt:
            event.accept()
            return
        reply = QMessageBox.question(self, 'Quit EyeShield', 'Are you sure you want to quit EyeShield?', QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
