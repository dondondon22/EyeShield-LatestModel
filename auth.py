...
USERNAME_PATTERN = re.compile('^[A-Za-z0-9_.-]{3,32}$')
...
class PasswordManager:
    ...
    def hash_password(password: str) -> str:
        ...
        digest = hashlib.pbkdf2_hmac('sha256', password.encode('utf-8'), salt, PasswordManager._ITERATIONS)
        return f'{PasswordManager._ALGO}${PasswordManager._ITERATIONS}${salt.hex()}${digest.hex()}'
...
class UserManager:
    ...
    _USER_COLUMNS = {'full_name': 'TEXT', 'display_name': 'TEXT', 'contact': 'TEXT', 'specialization': 'TEXT', 'availability_json': 'TEXT', 'preferred_timeout_minutes': 'INTEGER', 'is_active': 'INTEGER DEFAULT 1'}
    _PATIENT_RECORD_COLUMNS = {'archived_at': 'TEXT', 'archived_by': 'TEXT', 'archive_reason': 'TEXT', 'original_screener_username': 'TEXT', 'original_screener_name': 'TEXT', 'screened_at': 'TEXT', 'source_image_path': 'TEXT', 'heatmap_image_path': 'TEXT', 'image_sha256': 'TEXT', 'image_saved_at': 'TEXT', 'visual_acuity_left': 'TEXT', 'visual_acuity_right': 'TEXT', 'blood_pressure_systolic': 'TEXT', 'blood_pressure_diastolic': 'TEXT', 'fasting_blood_sugar': 'TEXT', 'random_blood_sugar': 'TEXT', 'diabetes_diagnosis_date': 'TEXT', 'symptom_blurred_vision': 'TEXT', 'symptom_floaters': 'TEXT', 'symptom_flashes': 'TEXT', 'symptom_vision_loss': 'TEXT', 'height': 'TEXT', 'weight': 'TEXT', 'bmi': 'TEXT', 'treatment_regimen': 'TEXT', 'prev_dr_stage': 'TEXT', 'ai_classification': 'TEXT', 'doctor_classification': 'TEXT', 'decision_mode': 'TEXT', 'override_justification': 'TEXT', 'final_diagnosis_icdr': 'TEXT', 'doctor_findings': 'TEXT', 'decision_by_username': 'TEXT', 'decision_at': 'TEXT'}
    _REFERRAL_HOSPITAL_COLUMNS = {'department': 'TEXT', 'contact_person': 'TEXT', 'phone': 'TEXT', 'email': 'TEXT', 'address': 'TEXT', 'is_active': 'INTEGER DEFAULT 1', 'is_default': 'INTEGER DEFAULT 0', 'created_at': 'TEXT', 'updated_at': 'TEXT'}
    _ACTIVITY_LOG_COLUMNS = {'event_type': 'TEXT', 'metadata_json': 'TEXT'}
    ...
    def _ensure_patient_record_columns(conn: sqlite3.Connection) -> None:
        ...
        for column_name, column_type in UserManager._PATIENT_RECORD_COLUMNS.items():
            if column_name in existing_columns:
                continue
            cur.execute(f'ALTER TABLE patient_records ADD COLUMN {column_name} {column_type}')
    ...
    def _migrate_users_json(conn: sqlite3.Connection) -> None:
        ...
        for user in users:
            ...
            specialization = UserManager._normalize_specialization(user.get('specialization'), role)
            ...
            cur.execute('\n                INSERT INTO users (\n                    username,\n                    full_name,\n                    display_name,\n                    contact,\n                    specialization,\n                    availability_json,\n                    password_hash,\n                    role\n                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)\n                ', (username, full_name or username, display_name or full_name or username, contact, specialization, availability_json, password_hash, role))
        conn.commit()
    ...
    def _is_valid_password(password: str) -> bool:
        if len(password) < MIN_PASSWORD_LENGTH:
            return False
        checks = [any((char.islower() for char in password)), any((char.isupper() for char in password)), any((char.isdigit() for char in password)), any((not char.isalnum() for char in password))]
        return all(checks)
    ...
    def resolve_username(username: str) -> str:
        ...
        cur = conn.cursor()
        try:
            cur.execute('SELECT username FROM users WHERE lower(username) = lower(?) LIMIT 1', (raw,))
            row = cur.fetchone()
    ...
    @staticmethod
    def _verify_admin_actor(conn: sqlite3.Connection, acting_username: Optional[str], acting_role: Optional[str], acting_password: Optional[str]) -> bool:
        if acting_role != ADMIN_ROLE or not acting_username or (not acting_password):
            return False
        cur = conn.cursor()
        cur.execute('SELECT password_hash, role FROM users WHERE username = ?', (acting_username,))
        row = cur.fetchone()
        if not row:
            return False
    ...
    @staticmethod
    def _verify_admin_identity(conn: sqlite3.Connection, acting_username: Optional[str], acting_role: Optional[str]) -> bool:
    ...
    def _load_global_inactivity_policy() -> tuple[bool, int]:
        ...
        try:
            ...
            if isinstance(loaded, dict):
                ...
                timeout_minutes = UserManager._clamp_timeout_minutes(loaded.get('inactivity_timeout_minutes', 15), default=15)
        except (OSError, json.JSONDecodeError):
            pass
        return (enabled, timeout_minutes)
    ...
    def _infer_event_from_legacy_action(action: str) -> tuple[str, dict[str, Any], str]:
        ...
        if prefix in {'ACCOUNT_CREATED', 'ACCOUNT_DELETED', 'ROLE_CHANGED', 'PASSWORD_RESET', 'USER_STATUS_CHANGED', 'USER_AVAILABILITY_UPDATED', 'SCREENED_PATIENT', 'RECORD_OPENED', 'RECORD_ARCHIVED', 'RECORD_RESTORED', 'REPORT_EXPORT_CSV', 'REPORT_GENERATED', 'REFERRAL_GENERATED', 'ACTIVITY_LOG_EXPORT_CSV'}:
        ...
        if lowered.startswith('assigned referral '):
            ...
            metadata = {'referral_id': referral_id.strip(), 'assigned_to': assignee.strip()}
        ...
        if lowered.startswith('updated referral '):
            ...
            metadata = {'referral_id': referral_id.strip(), 'from_status': from_status, 'to_status': to_status}
    ...
    @staticmethod
    def create_user(username: str, password: str, role: str='clinician', full_name: Optional[str]=None, display_name: Optional[str]=None, contact: Optional[str]=None, specialization: Optional[str]=None, availability_json: Optional[str]=None, acting_username: Optional[str]=None, acting_role: Optional[str]=None, acting_password: Optional[str]=None) -> bool:
        """Create a new user"""
        ...
        normalized_specialization = UserManager._normalize_specialization(specialization, normalized_role or '')
        ...
        try:
            cur.execute('\n                INSERT INTO users (\n                    username,\n                    full_name,\n                    display_name,\n                    contact,\n                    specialization,\n                    availability_json,\n                    is_active,\n                    password_hash,\n                    role\n                ) VALUES (?, ?, ?, ?, ?, ?, 1, ?, ?)\n                ', (username, full_name_value, display_name_value, contact_value, normalized_specialization or '', availability_json_value, pw_hash, normalized_role))
            conn.commit()
            success = True
        ...
        if PasswordManager.verify_password(password, pw_hash):
            if PasswordManager.needs_upgrade(pw_hash):
                with contextlib.suppress(sqlite3.Error):
                    ...
                    cur.execute('UPDATE users SET password_hash = ? WHERE id = ?', (upgraded_hash, user_id))
                    conn.commit()
            conn.close()
            return role
        conn.close()
    ...
    def get_user_profile(username: str) -> Optional[dict]:
        ...
        if not row:
            return None
        return {'username': row[0], 'full_name': row[1] or row[0], 'display_name': row[2] or row[1] or row[0], 'contact': row[3] or '', 'specialization': row[4] or '', 'availability_json': row[5] or '', 'preferred_timeout_minutes': row[6], 'role': row[7], 'is_active': bool(row[8])}
    ...
    def get_inactivity_policy(username: str) -> dict[str, Any]:
        ...
        if username:
            ...
            cur = conn.cursor()
            try:
                cur.execute('SELECT preferred_timeout_minutes FROM users WHERE username = ?', (username,))
                row = cur.fetchone()
                if row:
        ...
        return {'enabled': bool(global_enabled), 'default_minutes': int(default_minutes), 'user_minutes': user_minutes, 'effective_minutes': int(effective_minutes)}
    ...
    def get_all_users() -> list[tuple]:
        ...
        conn = get_connection()
        cur = conn.cursor()
        cur.execute('SELECT username, full_name, display_name, contact, specialization, availability_json, role, is_active FROM users')
        users = cur.fetchall()
        conn.close()
        return users

    @staticmethod
    def update_user_role(username: str, new_role: str, acting_username: Optional[str]=None, acting_role: Optional[str]=None, acting_password: Optional[str]=None) -> bool:
        ...
        if current_role == ADMIN_ROLE and normalized_role != ADMIN_ROLE and (UserManager._count_admins(conn) <= 1):
            ...
            return False
        try:
            cur.execute('UPDATE users SET role = ? WHERE username = ?', (normalized_role, username))
    ...
    @staticmethod
    def delete_user(username: str, acting_username: Optional[str]=None, acting_role: Optional[str]=None, acting_password: Optional[str]=None) -> bool:
        """Delete a user"""
        username = username.strip()
    ...
    @staticmethod
    def reset_password(username: str, new_password: str, acting_username: Optional[str]=None, acting_role: Optional[str]=None, acting_password: Optional[str]=None) -> bool:
    ...
    @staticmethod
    def update_user_availability(username: str, availability_json: str, acting_username: Optional[str]=None, acting_role: Optional[str]=None, acting_password: Optional[str]=None) -> bool:
        username = username.strip()
        if not username or not UserManager._can_manage_users(acting_role):
            return False
        conn = get_connection()
        ...
        if not UserManager._verify_admin_actor(conn, acting_username, acting_role, acting_password):
            ...
            return False
        try:
            cur.execute('UPDATE users SET availability_json = ? WHERE username = ?', (str(availability_json or ''), username))
            conn.commit()
            success = cur.rowcount > 0
        except sqlite3.Error:
            success = False
    ...
    def update_own_availability(current_username: str, availability_json: str) -> tuple[bool, str]:
        ...
        UserManager.add_activity_event(username=username, event_type='USER_AVAILABILITY_UPDATED', metadata={'target': username, 'scope': 'self'}, action_text='Availability Updated')
    ...
    def update_own_inactivity_timeout(current_username: str, timeout_minutes: Any) -> tuple[bool, str, int]:
        ...
        cur = conn.cursor()
        try:
            cur.execute('UPDATE users SET preferred_timeout_minutes = ? WHERE username = ?', (stored_value, username))
            conn.commit()
            success = cur.rowcount > 0
        except sqlite3.Error:
            success = False
        ...
        metadata = {'target': username, 'scope': 'self', 'requested_minutes': requested_minutes, 'effective_minutes': effective_minutes, 'default_minutes': default_minutes, 'auto_logout_enabled': bool(global_enabled)}
        UserManager.add_activity_event(username=username, event_type='INACTIVITY_TIMEOUT_PREFERENCE_UPDATED', metadata=metadata, action_text='Inactivity timeout preference updated')
        if requested_minutes > default_minutes:
            return (True, f'Saved. Your timeout was capped to {default_minutes} minute(s) by admin policy.', effective_minutes)
        return (True, 'Inactivity timeout preference updated.', effective_minutes)

    @staticmethod
    def update_own_account(current_username: str, current_password: str, new_display_name: str, new_username: Optional[str]=None, new_password: Optional[str]=None) -> tuple[bool, str, Optional[str]]:
        ...
        if target_new_password:
            ...
            if not UserManager._is_valid_password(target_new_password):
                conn.close()
                return (False, 'Password must be 12+ chars with uppercase, lowercase, number, and symbol.', None)
        ...
        UserManager.add_activity_event(username=target_username, event_type='PROFILE_UPDATED', metadata={'target': target_username}, action_text='Profile updated')
        return (True, 'Account updated successfully.', target_username)

    @staticmethod
    def add_activity_event(username: str, event_type: str, metadata: Optional[dict[str, Any] | str]=None, action_time: Optional[str]=None, action_text: Optional[str]=None) -> bool:
    ...
    def add_activity_log(username: str, action: str, action_time: Optional[str]=None) -> bool:
        ...
        return UserManager.add_activity_event(username=username, event_type=event_type, metadata=metadata, action_time=action_time, action_text=normalized_text)

    @staticmethod
    def get_activity_logs(from_time: Optional[str]=None, to_time: Optional[str]=None, query: Optional[str]=None, limit: int=100, offset: int=0, event_type: Optional[str]=None, username: Optional[str]=None, acting_username: Optional[str]=None, acting_role: Optional[str]=None) -> tuple[list[dict[str, Any]], int]:
        ...
        if from_text:
            if len(from_text) == 10:
                where_parts.append("COALESCE(date(action_time, 'localtime'), SUBSTR(action_time, 1, 10)) >= ?")
            else:
        ...
        if query_text:
            where_parts.append("(LOWER(username) LIKE ? OR LOWER(action) LIKE ? OR LOWER(COALESCE(event_type, '')) LIKE ? OR LOWER(COALESCE(metadata_json, '')) LIKE ?)")
        ...
        try:
            ...
            if not UserManager._verify_admin_identity(conn, acting_username, acting_role):
                conn.close()
                return ([], 0)
            cur.execute(f'SELECT COUNT(*) FROM activity_logs {where_sql}', tuple(params))
        ...
        for row in rows:
            ...
            if metadata_raw:
                try:
                    ...
                    metadata_value = {}
            entries.append({'username': str(row[0] or '').strip(), 'action': str(row[1] or '').strip(), 'time': str(row[2] or '').strip(), 'event_type': str(row[3] or 'LEGACY').strip().upper() or 'LEGACY', 'metadata': metadata_value})
        return (entries, total)

    @staticmethod
    def get_recent_activity(limit: int=120) -> list[tuple]:
        entries, _total = UserManager.get_activity_logs(limit=limit, offset=0)
        return [(entry.get('username', ''), entry.get('action', ''), entry.get('time', '')) for entry in entries]
    ...
    def list_clinicians(exclude_username: str='') -> list[dict]:
        ...
        for row in rows:
            ...
            clinicians.append({'username': username, 'display_name': display_name, 'specialization': str(row[3] or '').strip()})
        return clinicians

    @staticmethod
    def update_user_active_status(username: str, is_active: bool, acting_username: Optional[str]=None, acting_role: Optional[str]=None, acting_password: Optional[str]=None) -> bool:
